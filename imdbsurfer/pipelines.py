...
insertIntoGenre = 'INSERT INTO imdbsurfer_genre(dh_create, dh_update, name, user_create_id, user_update_id)' \
                  ' VALUES (now(), now(), %s, ({0}))'.format(selectUserByEmail)
...
insertIntoArtistRole = 'INSERT INTO imdbsurfer_artistrole(dh_create, dh_update, artist_id, role_id, user_create_id, user_update_id)' \
                       ' VALUES (now(), now(), ({0}), ({1}), ({2}))'.format(selectArtistByName, selectRoleByName, selectUserByEmail)
...
insertIntoMovie = 'INSERT INTO imdbsurfer_movie(name, year, rate, votes, link, metascore, minutes, watch, watched, dh_create, dh_update, user_create_id, user_update_id)' \
                  ' VALUES (%s, %s, %s, %s, %s, %s, %s, False, False, now(), now(), ({0}))'.format(selectUserByEmail)
...
insertIntoMovieGenre = 'INSERT INTO imdbsurfer_moviegenre(dh_create, dh_update, index, genre_id, movie_id, type_id, user_create_id, user_update_id)' \
                       ' VALUES (now(), now(), %s, ({0}), ({1}), ({2}), ({3}))'.format(selectGenreByName, selectMovieByLink, selectTypeByName, selectUserByEmail)
...
insertIntoMovieArtistRole = 'INSERT INTO imdbsurfer_movieartistrole(dh_create, dh_update, "artistRole_id", movie_id, user_create_id, user_update_id)' \
                            ' VALUES (now(), now(), ({0}), ({1}), ({2}))'.format(selectArtistRole, selectMovieByLink, selectUserByEmail)
...
class TypePipeline(object):
    ...
    def process_item(self, item, spider):
        ...
        if self.cursor.rowcount == 0:
            self.cursor.execute(insertIntoType, [_type, email])
...
class GenrePipeline(object):
    ...
    def process_item(self, item, spider):
        for genre in item['genres']:
            ...
            if self.cursor.rowcount == 0:
                self.cursor.execute(insertIntoGenre, [genre, email])
                self.connection.commit()
...
class RolePipeline(object):
    ...
    def process_item(self, item, spider):
        for role in [DIRECTOR, STAR]:
            ...
            if self.cursor.rowcount == 0:
                self.cursor.execute(insertIntoRole, [role, email])
                self.connection.commit()
...
class ArtistPipeline(object):
    ...
    def process_item(self, item, spider):
        for director in item['directors']:
            ...
            if self.cursor.rowcount == 0:
                self.cursor.execute(insertIntoArtist, [director, email])
                self.connection.commit()

            self.cursor.execute(selectArtistRole, [director, DIRECTOR])
            if self.cursor.rowcount == 0:
                self.cursor.execute(insertIntoArtistRole, [director, DIRECTOR, email])
                self.connection.commit()
        ...
        if item['stars'] is not None:
            for star in item['stars']:
                ...
                if self.cursor.rowcount == 0:
                    self.cursor.execute(insertIntoArtist, [star, email])
                    self.connection.commit()

                self.cursor.execute(selectArtistRole, [star, STAR])
                if self.cursor.rowcount == 0:
                    self.cursor.execute(insertIntoArtistRole, [star, STAR, email])
                    self.connection.commit()
    ...
    def process_item(self, item, spider):
        ...
        if self.cursor.rowcount == 0:
            self.cursor.execute(insertIntoMovie,
                                [item['name'], item['year'], rate, votes, link, metascore, item['minutes'], email])
        else:
        ...
        for _genre in item['genres']:
            ...
            if self.cursor.rowcount == 0:
                self.cursor.execute(insertIntoMovieGenre, [index, _genre, link, _type, email])
            else:
        ...
        for director in item['directors']:
            ...
            if self.cursor.rowcount == 0:
                self.cursor.execute(insertIntoMovieArtistRole, [director, DIRECTOR, link, email])
                self.connection.commit()
        ...
        if item['stars'] is not None:
            for star in item['stars']:
                ...
                if self.cursor.rowcount == 0:
                    self.cursor.execute(insertIntoMovieArtistRole, [star, STAR, link, email])
                    self.connection.commit()
